...
type TachibanaServiceClient interface {
	...
	Stream(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (TachibanaService_StreamClient, error)
}
...
func (c *tachibanaServiceClient) Stream(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (TachibanaService_StreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &TachibanaService_ServiceDesc.Streams[0], "/tachibanapb.TachibanaService/Stream", opts...)
	if err != nil {
		return nil, err
	}
	x := &tachibanaServiceStreamClient{stream}
...
type tachibanaServiceStreamClient struct {
	grpc.ClientStream
}

func (x *tachibanaServiceStreamClient) Recv() (*StreamResponse, error) {
...
type TachibanaServiceServer interface {
	...
	Stream(*StreamRequest, TachibanaService_StreamServer) error
...
func (UnimplementedTachibanaServiceServer) Stream(*StreamRequest, TachibanaService_StreamServer) error {
...
func _TachibanaService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TachibanaServiceServer).Stream(m, &tachibanaServiceStreamServer{stream})
}
...
type tachibanaServiceStreamServer struct {
	grpc.ServerStream
}

func (x *tachibanaServiceStreamServer) Send(m *StreamResponse) error {
